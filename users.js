...
const { roleAccess } = require('../utils/middlewares/auth');
...
router.get('/', roleAccess([1, 18]), (req, res, next) => {
    axios
        .get(process.env.SSO_API_URL + '/users', {
            params: req.query,
            headers: {
...
router.get('/:id', roleAccess([1, 18]), (req, res, next) => {
    ...
    axios
        ...
        .then(async (response) => {
            ...
            let roles_updated_at;
            let roles_updated_by;
            ...
            if (logistics_roles.length > 0) {
                ...
                if (creatorId) {
                    const {
                        ...
                        process.env.SSO_API_URL + '/users/' + creatorId,
                        {
                            params: req.query,
                            headers: {
                                Authorization: `Bearer ${process.env.SSO_API_KEY}@${process.env.SSO_API_SECRET}`,
        ...
        axios
            ...
            .then(async (response) => {
                ...
                return res.withStatus.ok(
                    'Successfully assign roles to user',
                    null,
                );
            })
            .catch((error) => {
